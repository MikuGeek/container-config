...
### 9. Stack targets belong in user systemd, not the Quadlet source directory

- Title: Put `<stack>.target` under `~/.config/systemd/user/`
- Context: `rsshub` stack target trial
- Problem: `systemctl --user daemon-reload` did not recognize `rsshub.target` when it was linked into `~/.config/containers/systemd/`.
- Cause: `~/.config/containers/systemd/` is consumed by the Podman Quadlet generator for `.container` and `.pod` sources, but plain systemd targets are loaded from the normal user systemd directory.
- Fix:
  - Keep the source file in `stacks/<stack>/<stack>.target`.
  - Link it into `~/.config/systemd/user/<stack>.target`.
  - Enable it via `~/.config/systemd/user/default.target.wants/<stack>.target`.
- Rule: Stack-level `.target` files must be installed through user systemd, not through the Quadlet source directory.

### 10. Stack targets can be the only default entrypoint

- Title: Use one target as the stack entrypoint
- Context: `rsshub` migration from per-unit `WantedBy=default.target`
- Problem: Managing pod, sidecar, app, and support services separately was noisy and easy to drift.
- Cause: Each Quadlet unit was installed directly into `default.target`, so there was no single stack-level control point.
- Fix:
  - Add `<stack>.target` with `Wants=` for the stack members.
  - Add `PartOf=<stack>.target` to the member `.pod` and `.container` files.
  - Remove member `WantedBy=default.target` lines.
  - Enable only `<stack>.target` for default startup.
- Rule: When a stack has a stable `.target`, use it as the only default entrypoint and keep member units out of `default.target`.
